...
- **`scripts/setup.mjs`**, **`scripts/discover.mjs`**, **`scripts/review.mjs`**, **`scripts/audit.mjs`**, and **`scripts/codegen.mjs`** support onboarding and manifest drafting: bootstrap `uat-manifest.yml` + npm scripts, scan UI/API routes (including Next.js route groups `(group)` and `basePath`), recommend diff-scoped UAT scope, report coverage gaps vs `flows[]` (with `deferred_coverage[]` ledger), and emit an optional Playwright skeleton under `.uat/generated/`.
